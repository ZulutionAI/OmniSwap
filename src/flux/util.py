...
def load_controlnet(name, device, transformer=None, controlnet_depth=6, is_inpainting=False, in_num=2):
    with torch.device(device):
        controlnet = ControlNetFlux(configs[name].params, controlnet_depth=controlnet_depth, is_inpainting=is_inpainting, in_num=in_num)
        # if is_inpainting is True:
        #     controlnet = ControlNetFlux(configs[name].params, controlnet_depth=controlnet_depth, is_inpainting=is_inpainting)
        # else:
...
def load_clip(device: str | torch.device = "cuda", max_length=77) -> HFEmbedder:
    return HFEmbedder("openai/clip-vit-large-patch14", max_length=max_length, torch_dtype=torch.bfloat16).to(device)
