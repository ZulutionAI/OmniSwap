    ...
    def forward(
        ...
        **kwargs
        ...
        if c_inp is not None:
            c_inp['img'] = self.img_in(c_inp['img'])
            c_vec = self.time_in(timestep_embedding(timesteps, 256))
            if self.params.guidance_embed:
                if guidance is None:
                    raise ValueError("Didn't get guidance strength for guidance distilled model.")
                c_vec = c_vec + self.guidance_in(timestep_embedding(guidance, 256))
            c_vec = c_vec + self.vector_in(c_inp['vec'])
            c_txt = self.txt_in(c_inp['txt'])
            c_inp['vec'] = c_vec
            c_inp['txt'] = c_txt
            c_ids = torch.cat((c_inp['txt_ids'], c_inp['img_ids']), dim=1)
            c_pe = self.pe_embedder(c_ids)
            c_inp['pe'] = c_pe
            c_inp['pe2'] = None
        ...
        for index_block, block in enumerate(self.double_blocks):
            ...
            if c_inp is not None:
                ...
                # print("c_inp['img'].shape", c_inp['img'].shape, "c_inp['txt'].shape", c_inp['txt'].shape)
            ...
            if self.training and self.gradient_checkpointing:
                ...
                if single_inject is not None:
                    vae_controlnet_hidden_states = [single_inject]
                elif c_inp is not None:
                    vae_controlnet_hidden_states = [c_inp['img']]
                else:
                ...
                img, txt = block(
                    ...
                    ip_atten_mask=ip_atten_mask,
                    lora_weight_list=lora_weight_list,
                    vae_controlnet_hidden_states=vae_controlnet_hidden_states,
                    **kwargs
                )
        ...
        for block in self.single_blocks:
            ...
            if c_inp is not None:
                c_img = torch.cat((c_inp['txt'], c_inp['img']), 1)
                c_out = block(c_img, vec=c_inp['vec'], pe=c_inp['pe'],
                            image_proj=image_proj,
                            ip_scale=ip_scale, 
                            ip_atten_mask=ip_atten_mask,
                            lora_weight_list=lora_weight_list,
                ...
                c_inp['img'] = c_out[:, c_inp['txt'].shape[1]:, ...]
                c_inp['txt'] = c_out[:, :c_inp['txt'].shape[1], ...]
        ...
        # c_inp['img'] = self.final_layer(c_inp['img'], c_inp['vec'])
        return img
